...
export const DesktopStockTable = React.memo<Props>(({error, isLoading, rows, sortDescriptor, onDetailPress, onSortChange, onStockDetailPress}) => {
        ...
        <Table className="hidden md:block">
            <Table.ScrollContainer>
                <Table.Content aria-label="Stock screener results" className="min-w-[980px]" sortDescriptor={sortDescriptor} onSortChange={onSortChange}>
                    ...
                    <Table.Body>{renderTableBody(rows, isLoading, error, chartTicker, onDetailPress, handleStockDetailPress, handleChartOpenChange)}</Table.Body>
                </Table.Content>
            </Table.ScrollContainer>
        </Table>
    );
